...
class ColorBasedLocalization:
    ...
    def mask_rcnn_callback(self, msg):   
        ...
        if moments["m00"] != 0:
            ...
            self.mask_rcnn_centroids.append((cX, cY))
        rospy.loginfo("Detected red object centroids: {}".format(self.mask_rcnn_centroids))
    ...
    def colormap_callback(self, msg):  
        ...
        self.find_matching_location_in_map(centroids)
    ...
    def find_matching_location_in_map(self, centroids):
        ...
        transform_matrix = self.get_transform_matrix( centroids )
    ...
    def get_transform_matrix(self, centroids):
        while len(self.mask_rcnn_centroids) < 2:
            print("Detected positions too few, waiting for more data...")
            time.sleep(1)  
        print("Detected positions:", self.mask_rcnn_centroids)
        
        centroids_2d = np.array(centroids)[:, :2]  # Keep only the first two columns (x, y)
        translation = np.mean(self.mask_rcnn_centroids, axis=0) - np.mean(centroids_2d, axis=0)


        
      
        map_angle = np.arctan2(centroids[1][1] - centroids[0][1], 
                               centroids[1][0] - centroids[0][0])

        detected_angle = np.arctan2(self.mask_rcnn_centroids[1][1] - self.mask_rcnn_centroids[0][1], 
                                    self.mask_rcnn_centroids[1][0] - self.mask_rcnn_centroids[0][0])
