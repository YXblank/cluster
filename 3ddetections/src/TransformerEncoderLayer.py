...
class TransformerEncoderLayer(nn.Module):
    ...
    def forward(self, src, input_key_padding_mask):
        ...
        input_key_padding_mask = input_key_padding_mask.to(dtype=torch.bool)
...
class TransformerDecoderLayer(nn.Module):
    ...
    def forward(self, global_input, input_key_padding_mask, position_embed):
        ...
        target2, global_attention_weights = self.multihead2(
        ...
        target = global_input + self.dropout2(target2)
        target = self.norm3(target)

        # Pass through linear layers with activation and dropout
        target2 = self.linear2(self.dropout(nn.functional.relu(self.linear1(target))))
    
        # Add residual connection from the linear transformation
        target = target + self.dropout3(target2)

        return target, global_attention_weights
...
class TransformerEncoder(nn.Module):
    ...
    def forward(self, input, input_key_padding_mask):
        ...
        if input_key_padding_mask.dtype == torch.uint8:  # If using byte tensor
            input_key_padding_mask = input_key_padding_mask.to(dtype=torch.bool)
