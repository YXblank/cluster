...
class MaskRCNN():
    ...
    #def load_weights(self, filepath, by_name=False, exclude=None): #? change from to
            ...
            for layer in layers:
                if layer.name in f:
                    ...
                    # Get weights from .h5 file
                        g = f[layer.name]
                        weight_names = [n for n in g.keys()]
                        weights = [g[n] for n in weight_names]
                        model_weights = layer.get_weights()

                    # Check shape compatibility
                        if len(weights) == len(model_weights):
                            for w, mw in zip(weights, model_weights):
                                if w.shape != mw.shape and skip_mismatch:
                                    print("Skipping {}: Shape mismatch {} vs {}".format(layer.name, w.shape, mw.shape))
                                    continue
                                elif w.shape != mw.shape:
                                    raise ValueError("Shape mismatch in {}: {} vs {}".format(layer.name, w.shape, mw.shape))
                            layer.set_weights(weights)
                            print("Loaded weights for {}".format(layer.name))
                        else:
                            print("Skipping {}: Weight count mismatch".format(layer.name))
                    except Exception as e:
                        print("Error loading {}: {}".format(layer.name, str(e)))
                else:
                    print("Layer {} not found in .h5 file".format(layer.name))
        else:
        # Default Keras loading (by structure)
