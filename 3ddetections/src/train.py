...
"""------------------------------------some settings----------------------------------------"""
conf = Config()
print('The PATH saved here:', conf.save_path)
...
dataloader_train = torch.utils.data.DataLoader(AG_dataset_train, shuffle=True, num_workers=2,
                                               collate_fn=cuda_collate_fn, pin_memory=False)
AG_dataset_test = AG(mode="test", datasize=conf.datasize, data_path=conf.data_path, filter_nonperson_box_frame=True,
                     filter_small_box=False if conf.mode == 'predcls' else True)
dataloader_test = torch.utils.data.DataLoader(AG_dataset_test, shuffle=False, num_workers=2,
                                              collate_fn=cuda_collate_fn, pin_memory=False)

gpu_device = 'cuda:0' if torch.cuda.is_available() else 'cpu:0'
...
for epoch in range(conf.nepoch):
    ...
    for b in range(len(dataloader_train)):
        ...
        im_data = data[0].cuda(0)
        im_info = data[1].cuda(0)
        gt_boxes = data[2].cuda(0)
        num_boxes = data[3].cuda(0)
        ...
        if not conf.bce_loss:
            ...
            spatial_label = torch.full([len(pred["spatial_gt"]), 6], -1, dtype=torch.long, device=attention_distribution.device)
            contact_label = torch.full([len(pred["contacting_gt"]), 17], -1, dtype=torch.long, device=attention_distribution.device)

            for i in range(len(pred["spatial_gt"])):
                spatial_label[i, : len(pred["spatial_gt"][i])] = torch.tensor(pred["spatial_gt"][i], device=attention_distribution.device)
                contact_label[i, : len(pred["contacting_gt"][i])] = torch.tensor(pred["contacting_gt"][i], device=attention_distribution.device)
